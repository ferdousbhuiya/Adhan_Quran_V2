...
const Adhan: React.FC<AdhanProps> = ({ location, settings, onUpdateSettings }) => {
  ...
  useEffect(() => {
    if (location) {
      ...
      fetchPrayerTimes(
        ...
        setTimes(data.times);
        // Update prayer times in notification scheduler
        notificationScheduler.updatePrayerTimes(data.times);
        notificationScheduler.updateEnabledPrayers(settings.notifications);
        setLoading(false);
      }).catch(err => {
  ...
  useEffect(() => {
    // Initialize notification scheduler when settings change
    notificationScheduler.initialize(settings.notifications);
    notificationScheduler.updateEnabledPrayers(settings.notifications);
